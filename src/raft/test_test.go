...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("Initialize leader successfully: %d",leader1)
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	DPrintf("leader: %d goes offline\n",leader1)
	cfg.checkOneLeader()
	...
	DPrintf("leader: %d goes \n",(leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("leader: %d goes online\n", (leader2+1)%servers)
	cfg.checkOneLeader()
...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("Peer %d goes online",(leader1 + 2) % servers)
	DPrintf("Peer %d goes online",(leader1 + 3) % servers)
	DPrintf("Peer %d goes online",(leader1 + 4) % servers)
	...
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
		DPrintf("Peer %d goes offline",i)
	}
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)
	DPrintf("Peer %d goes online",(leader1 + 0) % servers)
	DPrintf("Peer %d goes online",(leader1 + 1) % servers)
	DPrintf("Peer %d goes online",other)
	...
	DPrintf("All peers are connected")
	time.Sleep(2000*time.Millisecond)
...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		DPrintf("First,Trying to make an agreement of command %d on %d severs\n",10+index,servers)
		...
		DPrintf("Second,Trying to make an agreement of command %d on %d severs\n",10+index,servers-2)
		...
		//force the peer to restart
		cfg.start1((leader1 + 1) % servers)
		DPrintf("Peer %d restarted\n",(leader1+1) % servers)

		cfg.start1((leader1 + 2) % servers)
		DPrintf("Peer %d restarted\n",(leader1+2)%servers)

		cfg.connect((leader1 + 1) % servers)
		DPrintf("Peer %d goes online\n",(leader1+1) % servers)
		...
		DPrintf("Third,Trying to make an agreement of command %d on %d severs\n",10+index,servers-2)
		cfg.one(10+index, servers-2, true)
		index++
